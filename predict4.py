...
if __name__ == "__main__":
    ...
    for epochs in range(0, 10):
        ...
        for cancer_type, cases in stratified.items():
            if len(cases) < 64: continue
            test_data = batched(datagen.data(mode='val', cases=cases), batch_size=64)
            try:
                print (f"Validation {fetch.disease_lookup[cancer_type]}: {model.eval_data(test_data)}")
            except:
                print (f"Validation {fetch.disease_lookup[cancer_type]} failed")

        model.save("results/predict3.pth") 
